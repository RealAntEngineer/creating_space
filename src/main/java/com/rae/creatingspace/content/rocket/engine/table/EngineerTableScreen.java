...
public class EngineerTableScreen extends AbstractSimiContainerScreen<EngineerTableMenu> {
    ...
    private ScrollInput materialInput;
    ...
    private boolean         recipeAllowed = false;
    //int materialLevel = 0;
    ...
    protected void init() {
        ...
        engineThrustInput = new ScrollInput(x + 7, y + 172,
                ...
                .setState(getMenu().getSyncData().thrust())
        ...
        addRenderableWidget(materialInput);
        addRenderableWidget(materialLabel);
    ...
    private void syncWithBE() {
        ...
        try {
            ...
            syncData.putInt("materialLevel", materialInput.getState());
    ...
    private void craftEngine(BlockPos blockEntityPos, ResourceLocation propellantType, ResourceLocation exhaustType, ResourceLocation powerPackType, float isp, float mass, float thrust) {
        ...
        ItemStack engineBlueprint = ((EngineFabricationBlueprint) ItemInit.ENGINE_BLUEPRINT.get().asItem())
                .getBlueprintForEngine(engineSizeInput.getState(), expansionRatioSlider.getValueInt(), materialInput.getState(), (int) thrust, efficiency, propellantType, exhaustType, powerPackType);
        PacketInit.getChannel()
                .sendToServer(
                        EngineerTableCraft
