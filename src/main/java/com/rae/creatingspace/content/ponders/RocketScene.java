...
public class RocketScene {
    ...
    public static void rocketDebug(SceneBuilder builder,SceneBuildingUtil util ){
        CreateSceneBuilder scene = new CreateSceneBuilder(builder);
        ...
        scene.overlay().showOutlineWithText(flight_recorder,60).text("N/A");
        scene.idleSeconds(3);
        scene.addKeyframe();
        ...
        scene.overlay().showOutlineWithText(flight_recorder,40).text("not enough thrust");
        scene.idleSeconds(3);
        scene.addKeyframe();

        Selection additional_engines = util.select().fromTo(0,0,2,2,1,2);
        scene.world().showSection(additional_engines,Direction.UP);
        scene.overlay().showOutlineWithText(additional_engines,40).text("add more engines to solve the issue");
        scene.idleSeconds(3);
        scene.markAsFinished();
    }
}