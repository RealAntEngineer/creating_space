...
public class EngineMovementBehaviour implements MovementBehaviour {
    ...
    private static float flamme_width(float x) {
        return (float) (baseRadius * Math.cos(x / (maxDistance + 0.2f) * Math.PI / 2));
    }

    private static Color getColorExterior(float x) {
        return new Color(0xfff0b00f).mixWith(new Color(0x000fe9f0), Mth.clamp(0, 1 - x / maxDistance * 1.5f, 1));
    }

    private static Color getColorInterior(float x) {
        return new Color(0xff4fd2ff).mixWith(new Color(0x00e86642), Mth.clamp(0, x / maxDistance / 2, 1));
    }
